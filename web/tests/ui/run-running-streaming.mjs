...
import { runRunningStreamingStates } from './scenarios/running-streaming-states.mjs';
...
async function main() {
  ...
  if (!pnpmCommand) {
    throw new Error('pnpm not found; install pnpm to run isolated UI tests');
  }

  const sessionId = process.env.LUBAN_AGENT_BROWSER_SESSION ?? `luban-running-streaming-${crypto.randomBytes(6).toString('hex')}`;
  ...
  if (explicitBaseUrl) {
    ...
    logFile = path.join(os.tmpdir(), `luban-running-streaming-ui-${sessionId}.log`);
  ...
  try {
    if (dev) {
      ...
      if (dev.exitCode != null) {
        const devLog =
          logFile && fs.existsSync(logFile) ? fs.readFileSync(logFile, 'utf8') : '';
        const hasNextLockConflict = devLog.includes('Unable to acquire lock');
        if (!hasNextLockConflict) {
          process.stderr.write(`isolated ui test failed; log: ${logFile}\n`);
          throw new Error('web dev server exited before it became ready');
        }

        const fallbackBaseUrl = normalizeBaseUrl(
          process.env.LUBAN_AGENT_BROWSER_FALLBACK_BASE_URL ?? 'http://127.0.0.1:3000/',
        );
        process.stderr.write(`dev lock conflict detected, fallback to ${fallbackBaseUrl}\n`);
        await waitForHttpOk(fallbackBaseUrl, 10_000);
    ...
    await runRunningStreamingStates({ page, baseUrl });
  } catch (err) {
    if (logFile) {
      process.stderr.write(`isolated ui test failed; log: ${logFile}\n`);
    }
